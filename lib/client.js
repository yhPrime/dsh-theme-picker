...
`;
    ...
    async function activateThemePlugin(name) {
      // use-skin 处理主题排他性（停用其他主题并激活目标），但只接受服务端
      // installedThemeNames 识别的主题（registry 名匹配或 github: spec 匹配）。
      // git+https 安装的主题（如 deep-whale-day-night）会被拒绝，回退到
      // toggle 端点启用（1.29+ 的 toggle 对任意已安装插件可用，主题同样走排他）。
      try {
