...
`;
    ...
    async function activateThemePlugin(name) {
      // Exclusivity FIRST, across the full theme set our classifier knows —
      // regardless of whether the market's own classifier recognizes each one.
      // The market's use-skin only enforces single-active among ITS recognized
      // themes, so repo-matched skins (e.g. @dsh-external git+https installs)
      // would stay enabled when switching to a registry-named theme. Switching
      // every other installed theme off here first keeps exactly one active no
      // matter which sources the themes come from.
      ...
      // Enable the target: prefer use-skin (handles hot-mounted client-only
      // themes and the market's own bookkeeping); fall back to toggle for
      // packages the market's classifier misses (e.g. @dsh-external skins).
      try {
        await marketPost("/dsh-market/use-skin", { name });
      } catch (err) {
        const msg = err && err.message ? err.message : String(err);
        if (msg.indexOf("not an installed theme") === -1) throw new Error(msg);
    ...
    function apply(ctx) {
      ...
      function Row(props) {
        const { active, name, desc, meta, badgeText, badgeClass, actionLabel, busy, onAction, secondaryLabel, secondaryBusy, onSecondaryAction } = props;
